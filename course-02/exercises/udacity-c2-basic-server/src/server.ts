...
(async () => {
  ...
  app.post("/cars", async (req: Request, res: Response) => {
    let { make,type,cost,id,model } = req.body;
    if (!id || !model || !cost || !type) {
      res.status(400).send("Id, Model, Cost and Type are required");
    }
    const new_car: Car = {
      make: make, type: type, cost:cost, model: model, id:id
    }
