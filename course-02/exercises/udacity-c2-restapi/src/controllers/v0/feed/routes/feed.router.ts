...
router.get('/', async (req: Request, res: Response) => {
	...
	//Mapping the item with AWS getsignedURL for the URL stored in DB
	items.rows.map((item) => {
		if (item.url) {
			//Taking key from DB and get signedURL from S3 so resource can be accessed directly from our client
